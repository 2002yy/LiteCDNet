...
def BCEDiceLoss(inputs, targets):
    """
    专为极度不平衡的变化检测任务优化的 CE + Dice Loss
    inputs: 模型的原始输出 logits，形状 [B, 2, H, W]
    targets: 真实的掩码标签，形状 [B, 1, H, W] 或 [B, H, W]，其中 1 表示变化，0 表示未变化
    """
    # 1. 确保 targets 是 3D 的 [B, H, W] 以便计算交叉熵
    if targets.dim() == 4:
        targets = targets.squeeze(1)
    targets = targets.long()

    # ==========================================
    # 阶段 1：Cross Entropy Loss (处理像素级分类)
    # 相比 BCEWithLogits，CE 更适合 2 通道输出
    # ==========================================
    ce_loss = F.cross_entropy(inputs, targets)

    # ==========================================
    # 阶段 2：Foreground Dice Loss (仅针对变化类计算！)
    # 彻底解决模型全预测黑色的问题
    # ==========================================
    # 使用 softmax 获取两个通道的概率分布
    probs = torch.softmax(inputs, dim=1)
    
    # 【核心修复】：只取出通道 1（也就是变化类/前景）的概率图！！
    probs_pos = probs[:, 1, :, :]  # 形状: [B, H, W]
    targets_pos = targets.float()  # 形状: [B, H, W]，正好 1 是变化，0 是背景
    
    # 展平以便计算交集
    batch_size = targets_pos.shape[0]
    probs_pos = probs_pos.reshape(batch_size, -1)
    targets_pos = targets_pos.reshape(batch_size, -1)
    
    # 计算 Dice (平滑系数设大一点防止分母为0)
    smooth = 1e-5
    intersection = probs_pos * targets_pos
    dice = (2. * intersection.sum(1) + smooth) / (probs_pos.sum(1) + targets_pos.sum(1) + smooth)
    
    # Dice 越高越好，所以 Loss 是 1 - Dice。取一个 batch 的平均
    dice_loss = 1.0 - dice.mean()

    # ==========================================
    # 阶段 3：权重组合
    # 极大地提高 Dice 的比重，压制 CE 带来的背景主导问题
    # ==========================================
    # 推荐权重：CE 只要 0.2，把 Dice 提拔到 1.0
    loss = 0.2 * ce_loss + 1.0 * dice_loss
