...
if __name__ == '__main__':
    ...
    parser.add_argument('--project_name', default='LEVIR-A2Net2.0_ce_Adamw_0.0001_200', type=str)
    ...
    parser.add_argument('--net_G', default='A2Net2.0', type=str,
                        help='FC_EF | FC_Siam_conc | '
                             'FC_Siam_diff | UNet++|SNUNet|'
                             'DTCDSCN|IFNet|'
                             'base_transformer_pos_s4_dd8_dedim8|'
                             'ChangeFormer|'
                             'A2Net|A2Net2.0|DMINet|TFI-GR|'
                            'SEIFNet')
    ...
    # ==========================================
    # 🌟 自动保险逻辑：如果用到多输出网络，强制开启 deep_supervision
    # ==========================================
    if args.net_G in['A2Net', 'A2Net2.0', 'UNet++']:
        args.deep_supervision = True
        print(f"[*] Detected {args.net_G}, automatically enabled deep_supervision=True")
    ...
    # 将可视化结果保存的目录放在检查点目录下
    args.vis_dir = os.path.join(args.checkpoint_dir, 'vis')
    os.makedirs(args.vis_dir, exist_ok=True)

    # 启动训练
    train(args)
    
    # 如果你想跑完训练自动测试，把 test(args) 加上去
    # test(args)
